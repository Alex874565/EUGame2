...
public class VotingMinigameManager : MonoBehaviour
{
    [SerializeField] private Vector2 votesToWinRange = new Vector2(10, 20);
    [SerializeField] private float minigameDuration = 30f;
    ...
    private void Update()
    {
        ...
        timer.text = $"{Mathf.CeilToInt(minigameDuration - _minigameTimer)}s";
        if (_minigameTimer >= minigameDuration)
        {
            End(false);
        }
    ...
    {
        GameObject vote = Instantiate(votePrefab, transform);
        ...
        vote.transform.position = new Vector3(spawnX, spawnY, 0f);
